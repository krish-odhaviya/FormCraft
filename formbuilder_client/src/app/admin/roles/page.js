...
import { ArrowLeft, Plus, Loader2, X, Check, Trash2 } from "lucide-react";
import { api } from "@/lib/api/formService";
import { AuthGuard } from "@/components/auth/AuthGuard";
import { useAdminGuard } from "@/hooks/useAdminGuard";
import { validateRoleName, validateRoleDescription, formatServerErrors } from "@/lib/validation";
import { toast } from "react-hot-toast";

const RESTRICTED_MODULES = [
  "System Admin", "Module Management", "Role Management",
  "User Management", "All Access Requests",
];
...
function RolesContent() {
  const { isReady } = useAdminGuard(); // ← replaces copy-pasted admin check
  ...
  const selectRole = async (role) => {
    ...
    try {
      ...
      toast.error("Failed to load modules for this role.");
    }
  };
  ...
  const handleSaveRoleConfig = async () => {
    ...
    try {
      ...
      const serverErr = formatServerErrors(e.response?.data?.errors);
      toast.error(serverErr ?? e.response?.data?.message ?? "Failed to save role.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateRole = async () => {
    const nameErr = validateRoleName(newRole.roleName);
    if (nameErr) { toast.error(nameErr); return; }
    const descErr = validateRoleDescription(newRole.description);
    ...
    try {
      ...
      const serverErr = formatServerErrors(e.response?.data?.errors);
      toast.error(serverErr ?? e.response?.data?.message ?? "Failed to create role.");
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) => setNewRole((r) => ({
                  ...r,
                  roleName: e.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, ""),
                }))}
            ...
            <textarea
              ...
              rows={2}
              className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none resize-none focus:border-indigo-500"
            />
              ...
              <div className="grid grid-cols-2 gap-2">
                {PERMISSIONS.map((p) => (
                  <label key={p.key} className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={newRole[p.key]}
                      onChange={(e) => setNewRole((r) => ({ ...r, [p.key]: e.target.checked }))}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 border border-slate-200 text-slate-600 py-3 rounded-2xl text-sm font-bold hover:bg-slate-50"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center gap-6">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-sm font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200"
          >
            ...
            roles.map((role) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-2">
                  {role.isDefault && (
                    <span className="text-[9px] px-1.5 py-0.5 bg-emerald-50 text-emerald-600 rounded-full font-bold uppercase">Default</span>
                  )}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-black text-slate-900">
                  Configure <span className="text-indigo-600">{selectedRole.roleName}</span>
                </h2>
                <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {PERMISSIONS.map((p) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={selectedRole[p.key] || false}
