...
// ── Default form state ────────────────────────────────────────────────────────
const DEFAULT_FORM = {
  moduleName: "", description: "", prefix: "", iconCss: "",
  isParent: false, isSubParent: false, parentId: null, active: true, sortOrder: 0,
};
...
function ModuleModal({ initialData, modules, onSave, onClose }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(initialData?.id ? "Module updated" : "Module created");
      onSave();
    } catch (e) {
      const serverErr = formatServerErrors(e.response?.data?.errors);
      toast.error(serverErr ?? e.response?.data?.message ?? "Failed to save module.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto no-scrollbar">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl my-auto flex flex-col overflow-hidden border border-white/20">
        ...
        <div className="px-8 py-6 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2.5 text-slate-400 hover:text-slate-900 hover:bg-white rounded-2xl transition-all border border-transparent hover:border-slate-100">
            <X size={20} />
          </button>
        </div>
            ...
            <div className="grid grid-cols-2 gap-6 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100">
              <div className="space-y-2">
                ...
                >
                  ...
                  {modules
                    .filter((m) => (m.isParent || m.isSubParent) && m.id !== initialData?.id)
                    .map((m) => (
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center gap-6">
          ...
          <button
            onClick={() => setModal({})}
            className="ml-auto flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200"
          >
              ...
              <tbody className="divide-y divide-slate-100">
                {modules.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <code className="text-xs text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg">{m.prefix || "—"}</code>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => setModal({ initialData: m })} className="p-2 hover:bg-indigo-50 text-indigo-600 rounded-xl">
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => handleDelete(m.id)} className="p-2 hover:bg-red-50 text-red-500 rounded-xl">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
