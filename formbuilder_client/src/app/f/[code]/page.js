...
// Public form fill page — keyed by form code (slug), not UUID.
// Uses SRS-compliant /runtime/forms/{code} API endpoints.
...
function FormPageContent() {
  ...
  useEffect(() => {
    async function fetchFields() {
      try {
        const res = await api.getRuntimeForm(code);
        // console.log removed — was leaking full API response to browser console
        setFormDetails(res.data);
        if (res.data.status === "ARCHIVED") { setMessage("archived"); setLoading(false); return; }
        if (res.data.status !== "PUBLISHED") { setMessage("not_published"); setLoading(false); return; }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.submitByCode(code, visibleValues, formDetails.activeVersionId);
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      await api.createAccessRequest(formDetails.id, "VIEW_FORM", requestReason);
              ...
              <div className="pt-6 mt-8 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto items-center">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-8 py-3 rounded-xl text-sm font-bold shadow-lg transition-all bg-indigo-600 text-white hover:bg-indigo-700 active:scale-95 disabled:opacity-60"
                  >
                    {submitting ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                    {currentPage < formPages.length - 1 ? "Next Step" : submitting ? "Submitting..." : "Submit Form"}
                  </button>
                </div>
              </div>
            </form>
